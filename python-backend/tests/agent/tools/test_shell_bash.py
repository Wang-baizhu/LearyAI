...
async def test_background_command_returns_task_descriptor(shell_tool: Shell):
    """Test background command execution returns task metadata immediately."""

    class FakeManager:
        def create_bash_task(self, **kwargs):
    ...
    shell_tool._runtime.background_tasks = FakeManager()
    result = await shell_tool(Params(command="sleep 5", run_in_background=True))

    assert not result.is_error
    assert result.output == snapshot("")
    assert result.message == snapshot("Background task created with id `task_bash_test`.")
    assert result.extras == {"task_id": "task_bash_test", "status": "starting"}
    assert result.display is not None
    assert isinstance(result.display[0], ShellDisplayBlock)
    assert isinstance(result.display[1], BackgroundTaskDisplayBlock)

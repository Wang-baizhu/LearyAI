...
from kimi_cli.tools.agent import Agent
...
def test_agent_params_schema(agent_tool: Agent):
    ...
    assert agent_tool.base.parameters == snapshot(
        {
            "properties": {
                ...
                "run_in_background": {
                    "default": False,
                    "description": "暂未接入后台任务，传 true 会直接报错。",
                    "type": "boolean",
                },
                "timeout": {
                    "anyOf": [{"type": "integer"}, {"type": "null"}],
                    "default": None,
                    "description": "预留字段，当前不生效。",
                },
            },
    ...
    assert create_subagent_tool.base.parameters == snapshot(
        {
            "properties": {
                "name": {
                    "description": "Unique name for this agent configuration (e.g., 'summarizer', 'code_reviewer'). This name will be used to reference the agent in the Agent tool.",
                    "type": "string",
                },
                "system_prompt": {
    ...
    assert shell_tool.base.parameters == snapshot(
        {
            "properties": {
                ...
                "run_in_background": {
                    "default": False,
                    "description": "Whether to run the command as a background task. When true, the command returns a task id immediately and can be inspected with `TaskList` / `TaskOutput` / `TaskStop`.",
                    "type": "boolean",
                },
            },
            "required": ["command"],
    ...
    assert read_file_tool.base.parameters == snapshot(
        {
            "properties": {
                "path": {
                    "description": "The path to the file to read. The path must be within the working directory.",
                    "type": "string",
                },
                "line_offset": {
                    "default": 1,
    ...
    assert grep_tool.base.parameters == snapshot(
        {
            "properties": {
                ...
                "path": {
                    "default": ".",
                    "description": "File or directory to search in. Defaults to current working directory. The path must be within the working directory.",
                    "type": "string",
                },
                "glob": {
                    "anyOf": [{"type": "string"}, {"type": "null"}],
    ...
    assert write_file_tool.base.parameters == snapshot(
        {
            "properties": {
                "path": {
                    "description": "The path to the file to write. The path must be within the working directory.",
                    "type": "string",
                },
                "content": {
    ...
    assert str_replace_file_tool.base.parameters == snapshot(
        {
            "properties": {
                "path": {
                    "description": "The path to the file to edit. The path must be within the working directory.",
                    "type": "string",
                },
                "edit": {
                    "anyOf": [

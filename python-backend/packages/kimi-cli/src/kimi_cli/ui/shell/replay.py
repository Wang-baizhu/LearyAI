"""该文件职责：基于 wire 历史或 turn 录制结果重放最近对话过程。"""
...
from kimi_cli.wire.turn_record import TurnRecordingFile
...
async def _build_replay_turns_from_turn_recordings(wire_file: WireFile | None) -> list[_ReplayTurn]:
    _ = wire_file
    turn_file = TurnRecordingFile.current()
    if not turn_file.path.exists():
        return []

    turns: deque[_ReplayTurn] = deque(maxlen=MAX_REPLAY_TURNS)
    try:
        async for record in turn_file.iter_records():
            messages = record.to_wire_messages()
            if not messages or not isinstance(messages[0], TurnBegin):
                continue

            turn = _ReplayTurn(
                user_message=Message(role="user", content=messages[0].user_input),
                events=[],
            )
            for wire_msg in messages[1:]:
                if isinstance(wire_msg, TurnEnd):
                    continue
                if not is_event(wire_msg):
                    continue
                if isinstance(wire_msg, StepBegin):
                    turn.n_steps = wire_msg.n
                turn.events.append(wire_msg)
            turns.append(turn)
    except Exception:
        logger.exception(
            "Failed to build replay turns from turn recording file {file}:",
            file=turn_file.path,
        )
        return []
    return list(turns)

...
class Approval:
    def __init__(
        ...
        *,
        ...
        self._auto_approve_actions: set[str] = set(auto_approve_actions or set())
        """Set of action names that should automatically be approved."""

    async def _persist(self) -> None:
        if self._session is None:
            return
        self._session.state.approval.yolo = self._yolo
        self._session.state.approval.auto_approve_actions = set(self._auto_approve_actions)
        await self._session.save_state()

    async def set_yolo(self, yolo: bool) -> None:
        self._yolo = yolo
        await self._persist()
    ...
    async def fetch_request(self) -> Request:
        ...
        while True:
            ...
            if request.action in self._auto_approve_actions:
                ...
                await self.resolve_request(request.id, "approve")
                continue

            return request

    async def resolve_request(self, request_id: str, response: Response) -> None:
        """

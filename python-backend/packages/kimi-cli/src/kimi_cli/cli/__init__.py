# 该文件职责：提供 Kimi CLI 的命令行入口与子命令分发。
...
def _normalize_env_text(value: str | None) -> str | None:
    if value is None:
        return None
...
def _build_env_runtime_context() -> tuple[str | None, str | None]:
    user_id = _normalize_env_text(os.getenv("LEARY_USER_ID"))
    kb_id = _normalize_env_text(os.getenv("LEARY_KB_ID"))
...
def kimi(
    ...
    if os.getenv("KIMI_DEBUGPY") in {"1", "true", "yes", "on"}:
        host = os.getenv("KIMI_DEBUGPY_HOST", "127.0.0.1").strip() or "127.0.0.1"
        port_raw = os.getenv("KIMI_DEBUGPY_PORT", "5678").strip() or "5678"
        try:
            port = int(port_raw)
        except ValueError as exc:
            raise typer.BadParameter(
                f"Invalid KIMI_DEBUGPY_PORT: {port_raw}",
                param_hint="KIMI_DEBUGPY_PORT",
            ) from exc
        try:
            import debugpy
        except ModuleNotFoundError as exc:
            raise RuntimeError(
                "debugpy is required for KIMI_DEBUGPY. Install it or disable KIMI_DEBUGPY."
            ) from exc
        debugpy.listen((host, port))
        debugpy.wait_for_client()
    load_dotenv(".env.agent.local", override=False)
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, bool]:
        """
        ...
        match ui:
            case "shell":
                succeeded = await instance.run_shell(prompt, prefill_text=prefill_text)
            case "print":
    ...
    async def _post_run(last_session: Session, succeeded: bool) -> None:
        if not succeeded:
            return
        await last_session.finalize_run()

    async def _reload_loop(session_id: str | None):
        prefill_text: str | None = None
        while True:
            try:
                last_session, succeeded = await _run(session_id, prefill_text=prefill_text)
                break
            except Reload as e:
    ...
    env_user_id, env_kb_id = _build_env_runtime_context()
    runtime_token = None
    if env_user_id is not None or env_kb_id is not None:
        runtime_token = set_current_context(
            RuntimeContext(user_id=env_user_id, project_id=None, kb_id=env_kb_id)
        )
    try:
        asyncio.run(_reload_loop(session_id))
    finally:
        if runtime_token is not None:
            reset_current_context(runtime_token)
...
def acp(
    host: Annotated[
        ...
        typer.Option(
            "--host",
            help="Host to bind for ACP WebSocket server. Default: 127.0.0.1",
        ),
    ] = "127.0.0.1",
    port: Annotated[
        int,
        typer.Option(
            "--port",
            help="Port to bind for ACP WebSocket server. Default: 8010",
        ),
    ] = 8010,
    transport: Annotated[
        Literal["ws", "stdio"],
        typer.Option(
            "--transport",
            help="ACP transport to use. Default: ws",
        ),
    ] = "ws",
):
    ...
    acp_main(transport=transport, host=host, port=port)

...
from typing import TYPE_CHECKING, Any, Literal, get_type_hints, overload
...
class KimiToolset:
    ...
    def load_tools(self, tool_paths: list[str], dependencies: dict[type[Any], Any]) -> None:
        ...
        logger.debug("Loaded tools: {good_tools}", good_tools=good_tools)
        if bad_tools:
    ...
    def load_tool_classes(
        ...
        for tool_cls in tool_classes:
            tool = self._instantiate_tool_class(tool_cls, dependencies)
            if tool is not None:
                self.add(tool)
    ...
    def _load_tool(tool_path: str, dependencies: dict[type[Any], Any]) -> ToolType | None:
        ...
        if tool_cls is None:
            return None
        return KimiToolset._instantiate_tool_class(tool_cls, dependencies)

    @staticmethod
    def _instantiate_tool_class(tool_cls: type[Any], dependencies: dict[type[Any], Any]) -> ToolType | None:
        args: list[Any] = []
        if "__init__" in tool_cls.__dict__:
            type_hints = get_type_hints(tool_cls.__init__, globalns=vars(importlib.import_module(tool_cls.__module__)))

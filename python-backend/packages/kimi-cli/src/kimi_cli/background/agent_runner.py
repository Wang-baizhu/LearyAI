...
class BackgroundAgentRunner:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            if isinstance(exc.__cause__, asyncio.CancelledError):
                ...
                await self._runtime.subagent_store.update_instance(self._agent_id, status="failed")
            ...
            await self._runtime.subagent_store.update_instance(self._agent_id, status="killed")
            ...
            if current_task is not None:
                self._runtime.run_handle_registry.unregister(self._agent_id, task=current_task)
    ...
    async def _run_core(self, output: SubagentOutputWriter) -> None:
        ...
        record = await self._runtime.subagent_store.require_instance(self._agent_id)
        ...
        try:
            async def _ui_loop_fn(wire: Wire) -> None:
                ...
                while True:
                    ...
                    if self._runtime.background_agent_message_bridge is not None:
                        await self._runtime.background_agent_message_bridge(
                            self._agent_id,
                            self._subagent_type,
                            msg,
                        )
            ...
            final_response, failure = await run_with_summary_continuation(
                ...
                prompt,
                _ui_loop_fn,
                self._runtime.subagent_store.wire_target(self._agent_id),
                self._cancel_event,
            )
            ...
            await self._runtime.subagent_store.update_instance(self._agent_id, status="idle")
            self._manager._mark_task_completed(self._task_id)
        finally:
            self._runtime.run_state_registry.leave(self._agent_id)

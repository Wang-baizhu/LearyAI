...
def default_store_kind(monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest) -> None:
    """Default to file store in tests unless PG-specific fixture is explicitly used."""
    if "pg_env" in request.fixturenames:
        return
    monkeypatch.setenv("LEARY_STORE", "file")


@pytest.fixture(autouse=True)
def isolated_home(monkeypatch: pytest.MonkeyPatch) -> Generator[None]:
    """Isolate Path.home()/~/.kimi access so tests never touch the real user directory."""
...
def session(temp_work_dir: KaosPath, temp_share_dir: Path) -> Session:
    ...
    return Session(
        ...
        wire_file=WireFile(
            target=StoreTarget(
                kind="session", session_id="test", path=temp_share_dir / "wire.jsonl"
            )
        ),
        state=SessionState(),
        title="Test Session",
...
@pytest.fixture
def approval(session: Session) -> Approval:
    """Create a Approval instance."""
    return Approval(yolo=True, session=session)
...
def runtime(
    ...
    rt.root_wire_hub = RootWireHub()
    rt.approval_runtime = ApprovalRuntime()
    rt.approval_runtime.bind_root_wire_hub(rt.root_wire_hub)
    rt.notifications = NotificationManager(session.context_file.parent / "notifications", config.notifications)
    rt.background_tasks = BackgroundTaskManager(
        session,
        config.background,
        notifications=rt.notifications,
    )
    rt.background_tasks.bind_runtime(rt)
    rt.hook_engine = HookEngine(
        hooks=config.hooks,
        cwd=str(session.work_dir),
    )
    rt.subagent_store = SubagentStore(session)

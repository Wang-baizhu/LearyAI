    ...
    async def start_server(
        ...
        *,
        ...
        try:
            ...
            pytest.skip(f"local socket bind unavailable in current environment: {exc}")
...
this content is from local mock server.
"""
    server_url = await mock_http_server(page_markdown, content_type="text/markdown; charset=utf-8")
    result = await fetch_url_tool(Params(url=f"{server_url}/"))

    assert not result.is_error
    assert result.output == snapshot(page_markdown)
...
async def test_fetch_url_invalid_url(fetch_url_tool: FetchURL) -> None:
    ...
    result = await fetch_url_tool(Params(url="http://127.0.0.1:1/"))
...
async def test_fetch_url_empty_url(fetch_url_tool: FetchURL) -> None:
    ...
    # Should fail
    assert result.is_error
    assert result.message == snapshot("URL cannot be empty.")
...
async def test_fetch_url_javascript_driven_site(
    ...
    js_only_html = """\
<html>
  <head><title>JS Only</title></head>
  <body><script>document.write('dynamic')</script></body>
</html>
"""
    server_url = await mock_http_server(js_only_html, content_type="text/html; charset=utf-8")
    result = await fetch_url_tool(Params(url=f"{server_url}/"))

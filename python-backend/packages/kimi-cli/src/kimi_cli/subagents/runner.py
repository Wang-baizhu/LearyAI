...
async def run_soul_checked(
    ...
    try:
        await run_soul(
            ...
            ui_loop_fn,
            cancel_event,
            wire_file=WireFile(target=wire_target),
...
class ForegroundRunRequest:
    ...
    model: str | None
    resume: str | None
    on_wire_message: Callable[[object], Awaitable[None]] | None = None
    on_wire_created: Callable[[Wire], Awaitable[None]] | None = None
    mirror_request_messages_to_parent: bool = True
...
class ForegroundSubagentRunner:
    ...
    async def run(self, req: ForegroundRunRequest) -> ToolReturnValue:
        ...
        await self._store.update_instance(
            agent_id,
            status="running_foreground",
            description=req.description.strip(),
        )
        self._runtime.run_state_registry.enter(
            target_id=agent_id,
            kind="subagent",
            mode="running_foreground",
            parent_session_id=self._runtime.session.id,
            subagent_type=actual_type,
            name=req.description.strip() or prepared.record.description,
        )
        ...
        if current_task is not None:
            self._runtime.run_handle_registry.register(
                target_id=agent_id,
                kind="subagent",
                mode="running_foreground",
                task=current_task,
        ...
        try:
            ...
            ui_loop_fn = self._make_ui_loop_fn(
                ...
                on_wire_message=req.on_wire_message,
                on_wire_created=req.on_wire_created,
                mirror_request_messages_to_parent=req.mirror_request_messages_to_parent,
            )
            ...
            final_response, failure = await run_with_summary_continuation(
                ...
                prompt,
                ui_loop_fn,
                self._store.wire_target(agent_id),
                cancel_event,
            )
            if failure is not None:
                await self._store.update_instance(agent_id, status="failed")
            ...
            await self._store.update_instance(agent_id, status="killed")
            ...
            if current_task is not None:
                self._runtime.run_handle_registry.unregister(agent_id, task=current_task)
            self._runtime.run_state_registry.leave(agent_id)
        ...
        await self._store.update_instance(agent_id, status="idle")
    ...
    async def _prepare_instance(self, req: ForegroundRunRequest) -> PreparedInstance:
        if req.resume:
            record = await self._store.require_instance(req.resume)
        ...
        record = await self._store.create_instance(
            agent_id=agent_id,
            description=req.description.strip(),
        ...
        async def _ui_loop_fn(wire: Wire) -> None:
            ...
            while True:
                ...
                if isinstance(
                    ...
                    if not mirror_request_messages_to_parent:
                        continue

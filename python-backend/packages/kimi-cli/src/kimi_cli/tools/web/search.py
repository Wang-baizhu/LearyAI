...
class SearchWeb(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        ...
        if not query:
            return builder.error("Search query cannot be empty.", brief="Empty query")
        params = params.model_copy(update={"query": query})
        ...
        try:
            ...
            return builder.error(
                f"Failed to search due to timeout after {params.timeout}s.",
                brief="Search request timed out",
            )
        except aiohttp.ClientError as e:
            return builder.error(
                f"Failed to search due to network error: {e}",

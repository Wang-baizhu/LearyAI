...
def test_agent_description(agent_tool: Agent):
...
- Created agents can be referenced by name in the Agent tool
...
2. Use the Agent tool with `subagent_type='code_reviewer'` to launch the created agent
"""
    )
...
When `run_in_background=true`, the tool returns immediately with a background task id instead of waiting for command completion.
...
- Use `run_in_background=true` only for commands that can safely continue without immediate output, and only from the root agent.

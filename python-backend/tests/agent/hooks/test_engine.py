"""该文件职责：验证 HookEngine 的匹配、去重与失败开放行为。"""
...
@pytest.mark.asyncio
async def test_match_tool_name(engine: HookEngine) -> None:
...
@pytest.mark.asyncio
async def test_block(engine: HookEngine) -> None:
...
@pytest.mark.asyncio
async def test_no_match(engine: HookEngine) -> None:
    results = await engine.trigger("PreToolUse", matcher_value="Grep", input_data={})
    assert results == []


@pytest.mark.asyncio
async def test_dedup_identical_commands() -> None:
    engine = HookEngine(
        [
...
@pytest.mark.asyncio
async def test_invalid_regex_skips_hook() -> None:
    engine = HookEngine(

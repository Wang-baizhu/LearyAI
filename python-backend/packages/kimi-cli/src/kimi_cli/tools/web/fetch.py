"""该文件职责：抓取网页正文，并在服务接口与本地提取之间做统一收敛。"""
...
class FetchURL(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        ...
        if not params.url.strip():
            ...
            return builder.error("URL cannot be empty.", brief="Empty URL")
        params = params.model_copy(update={"url": url})
    ...
    async def fetch_with_http_get(params: Params) -> ToolReturnValue:
        ...
        try:
            ...
            return builder.error(
                (
                    f"Failed to fetch URL due to timeout after {params.timeout}s. "
                    "This may indicate the server is slow or unreachable."
                ),
    ...
    async def _fetch_with_service(self, params: Params) -> ToolReturnValue:
        ...
        try:
            ...
            return builder.error(
                (
                    f"Failed to fetch URL via service due to timeout after {params.timeout}s. "
                    "This may indicate the service is unreachable."
                ),
                brief="Fetch service timed out",
            )

...
class Params(BaseModel):
    ...
    path: str = Field(
        description=(
            ...
            "The path must be within the working directory."
        ),
        default=".",
    )
    glob: str | None = Field(
...
class Grep(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        try:
            ...
            work_dir = KaosPath.cwd().canonical()
            raw_path = KaosPath(params.path).expanduser()
            resolved_path = (
                raw_path.canonical()
                if raw_path.is_absolute()
                else (work_dir / raw_path).canonical()
            )
            if not is_within_directory(resolved_path, work_dir):
                return ToolError(
                    message=f"`{params.path}` is outside the working directory.",
                    brief="Invalid path",
                )
            ...
            rg = ripgrepy.Ripgrepy(params.pattern, str(resolved_path), rg_path=rg_path)
